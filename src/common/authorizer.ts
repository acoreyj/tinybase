...
export const checkMergeableChanges = <withHashes extends boolean>(
	...
	try {
		...
		objForEach(tablesObj, (tableStamp, tableId) => {
			...
			const tableExists = store ? storeHasTable(store, tableId) : false;
			const action = tableExists ? "update" : "create";
			const tableAuthRule = tableSchemaDef.tableAuthorization?.[action] as
				| AuthFunctionName
				| undefined;
			const tableAuthorized = tableAuthRule
				? !!serverFunctions.authorization[tableAuthRule]?.(authContext, {
			...
			if (!tableAuthorized) {
				logger(
					`checkMergeableChanges: Table "${tableId}" blocked (no matching allow rule for ${action}) (${tableAuthRule})`,
				);
				authorized = false;
				return;
			}
			...
			objForEach(rowStampsObj, (rowStamp, rowId) => {
				...
				objForEach(cellStampsObj, (cellStamp, cellId) => {
					const cellExists = store
						? storeHasCell(store, tableId, rowId, cellId)
						: false;
					const action = cellExists ? "update" : "create";
					logger(`checkMergeableChanges: Cell action determined`, {
						tableId,
						rowId,
						cellId,
						cellExists,
					...
					const cellSchema = tableSchemaDef.rowSchema?.[cellId];
					const cellAuthRule = cellSchema?.authorization?.[action] as
						| AuthFunctionName
						| undefined;

					const cellAuthorized = cellAuthRule
						? !!serverFunctions.authorization[cellAuthRule]?.(authContext, {
								tablesStamp,
								tableStamp,
								tableId,
					...
					if (!cellAuthorized) {
						logger(
							`checkMergeableChanges: Cell "${tableId}.${rowId}.${cellId}" blocked (no matching allow rule for ${action})`,
						);
						authorized = false;
					}
				});
...
const storeHasTable = (
	...
	typeof (store as MergeableStoreEnhanced).hasTable === "function"
		? (store as MergeableStoreEnhanced).hasTable(tableId)
		: !!(store as MergeableStore).getTable?.(tableId);
...
const storeHasCell = (
	...
	typeof (store as MergeableStoreEnhanced).hasCell === "function"
		? (store as MergeableStoreEnhanced).hasCell(tableId, rowId, cellId)
		: !!(store as MergeableStore).getCell?.(tableId, rowId, cellId);
